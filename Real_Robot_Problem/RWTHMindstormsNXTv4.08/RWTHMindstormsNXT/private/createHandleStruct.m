...
%% Create & initialize handle struct
    ...
    h.IRSensorVersion = @IRSensorVersion;
...
%% --- NESTED FUNCTION IRSensorVersion
    function ret = IRSensorVersion(port, val)
        if nargin > 1
            v_IRSensorVersion{port+1} = val;
        end%if
        ret = v_IRSensorVersion{port+1};
    end%function
